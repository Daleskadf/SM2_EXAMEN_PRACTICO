    ...
    try {
      ...
      if (email == 'admin@virtual.upt.pe') {
        ...
        try {
          ...
          // Si falla la actualización no impedimos el login; lo registramos para debugging
          debugPrint('No se pudo actualizar rol a admin: $e');
        }
      }
      ...
      // -------------------------
      try {
        final device = kIsWeb ? 'Web' : Theme.of(context).platform.toString();
        await FirebaseFirestore.instance.collection('login_history').add({
          'userId': userCredential.user!.uid,
          'email': email,
          'timestamp': FieldValue.serverTimestamp(),
          'device': device,
          'ip': '', // Opcional: rellena desde un servicio externo si quieres
        });

        // Actualizar lastLogin en users (no obligatorio pero útil)
        await FirebaseFirestore.instance
            .collection('users')
            .doc(userCredential.user!.uid)
            .update({'lastLogin': FieldValue.serverTimestamp(), 'updatedAt': FieldValue.serverTimestamp()});
      } catch (e) {
        debugPrint('Error al registrar login_history o actualizar lastLogin: $e');
        // No hacemos fallar el login por esto; simplemente lo registramos en logs
      }
      ...
      // Antes de navegar, aseguramos que _isLoading vuelva a false
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
